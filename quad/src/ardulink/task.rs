...
use mavlink::ardupilotmega::{
    MavMessage, MavModeFlag, MavSeverity, HEARTBEAT_DATA, STATUSTEXT_DATA,
};
...
impl MavlinkTask {
    ...
    fn publish_message(&self, msg: &MavMessage, tx: &TaskChannel) -> Result<(), Error> {
    ...
    fn decode_mode_flag(&self, flag: MavModeFlag) -> AutopilotStatus {
        AutopilotStatus {
            custom_mode_enabled: flag.intersects(MavModeFlag::MAV_MODE_FLAG_CUSTOM_MODE_ENABLED),
            test_enabled: flag.intersects(MavModeFlag::MAV_MODE_FLAG_TEST_ENABLED),
            auto_enabled: flag.intersects(MavModeFlag::MAV_MODE_FLAG_AUTO_ENABLED),
            guided_enabled: flag.intersects(MavModeFlag::MAV_MODE_FLAG_GUIDED_ENABLED),
            stabilize_enabled: flag.intersects(MavModeFlag::MAV_MODE_FLAG_STABILIZE_ENABLED),
            hil_enabled: flag.intersects(MavModeFlag::MAV_MODE_FLAG_HIL_ENABLED),
            manual_input_enabled: flag.intersects(MavModeFlag::MAV_MODE_FLAG_MANUAL_INPUT_ENABLED),
            safety_armed: flag.intersects(MavModeFlag::MAV_MODE_FLAG_SAFETY_ARMED),
        }
    }

    /// Process a heartbeat message
    fn process_heartbeat(&self, heartbeat: &HEARTBEAT_DATA, tx: &TaskChannel) -> Result<(), Error> {
        // Decode the mode flags
        let status = self.decode_mode_flag(heartbeat.base_mode);

        let pub_packet = publish!(
            "mavlink/reproc/heartbeat_armed",
            &HeartbeatFlag {
                value: status.safety_armed
            }
        );
        tx.send(pub_packet)?;
...
impl Task for MavlinkTask {
    fn init(&mut self, tx: TaskChannel, meta_tx: MetaTaskChannel) -> Result<(), Error> {
        info!(
            "MavlinkTask initializing with connection: {}",
