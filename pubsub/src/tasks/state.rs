...
impl RunnerState {
    ...
    pub fn get_topic_row_count(&self, topic: &str) -> Option<usize> {
        self.logs
            .get(topic)
            .map(|record| record.to_record_batch().num_rows())
    }
...
mod tests {
    ...
    fn test_get_latest_topic_data() {
        ...
        state
            .apply_record(&publish!("test_topic", &test_data))
            .unwrap();
        state
            .apply_record(&publish!("test_topic", &test_data2))
            .unwrap();
        state
            .apply_record(&publish!("test_topic", &test_data3))
            .unwrap();
