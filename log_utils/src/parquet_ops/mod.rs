...
pub fn find_parquet_files(dir: &Path, recursive: bool, filter: Option<&str>) -> Result<Vec<PathBuf>> {
    ...
    for file_path in input_files {
        match read_parquet_file(file_path) {
                    ...
                    match batch_result {
                        Ok(batch) => {
                            if let Err(e) = writer.write(&batch) {
                                eprintln!("Warning: Failed to write batch from {}: {}", file_path.display(), e);
                                if !force_merge {
                        ...
                        Err(e) => {
                            eprintln!("Warning: Failed to read batch from {}: {}", file_path.display(), e);
                            if !force_merge {
            ...
            Err(e) if force_merge => {
                eprintln!("Warning: Skipping file {} due to error: {}", file_path.display(), e);
            },
            Err(e) => return Err(e),
        }
    }
...
fn schemas_compatible(schema1: &arrow::datatypes::Schema, schema2: &arrow::datatypes::Schema) -> bool {
...
pub fn merge_parquet_files_by_schema_groups(
    ...
    for file_path in input_files {
        ...
        schema_groups.entry(_schema_key)
            .or_default()
