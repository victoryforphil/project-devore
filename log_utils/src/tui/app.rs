...
use ratatui::{
    ...
    text::{Line, Span},
    widgets::{Block, Borders, Cell, List, ListItem, Paragraph, Row, Table, Tabs, Scrollbar, ScrollbarOrientation, ScrollbarState},
    Terminal, Frame,
};
...
impl App {
    ...
    fn ensure_selected_file_visible(&mut self) {
        ...
        let mut dir_map: std::collections::HashMap<PathBuf, Vec<PathBuf>> = std::collections::HashMap::new();
        ...
        for path in &self.parquet_files {
            if let Some(parent) = path.parent() {
                ...
                dir_map.entry(parent_path.clone())
                    ...
                    match key.code {
                        ...
                        KeyCode::End => {
                            if app.selected_tab == 0 && !app.parquet_files.is_empty() {
                                ...
                                app.file_browser_scroll = app.parquet_files.len().saturating_sub(10);
                            } else if let Some(batch) = &app.current_batch {
                                if batch.num_rows() > 0 {
                                    app.current_row = batch.num_rows() - 1;
                                    app.scroll_offset = app.current_row.saturating_sub(app.max_rows_per_page) + 1;
                                }
                            }
                        },
                        _ => {}
...
fn render_file_browser(f: &mut Frame, app: &App, area: Rect) {
    ...
    for dir in all_dirs.iter() {
        ...
        let dir_line = Line::from(vec![
            ...
            Span::styled(dir_display, Style::default().fg(Color::Blue).add_modifier(Modifier::BOLD)),
        ]);
        ...
        if let Some(files) = dir_map.get(dir) {
            ...
            for path in sorted_files.iter() {
                let global_file_idx = app.parquet_files.iter().position(|p| p == path).unwrap_or(0);
                ...
                let file_line = Line::from(vec![
                    ...
                    Span::styled(" | ", Style::default().fg(Color::DarkGray)),
                    Span::styled(format!("path: {}", path.display()), Style::default().fg(Color::DarkGray)),
                ]);
    ...
    let items_slice = items.clone().into_iter()
        .skip(start_idx)
    ...
    let list = List::new(items_slice)
        .block(Block::default()
            .title(format!("Parquet Files ({} files, {} dirs)", 
                app.parquet_files.len(), all_dirs.len()))
            .borders(Borders::ALL))
    ...
    if items.clone().len() > list_height {
        ...
        let scrollbar_state = ScrollbarState::new(items.len())
            .position(app.file_browser_scroll);
    ...
    match &app.current_batch {
        Some(batch) => {
            ...
            let mut header_text = vec![
                format!("File: {}", app.parquet_files[app.selected_file_index].display()),
            ...
            let header_cells = headers.iter()
                .map(|h| Cell::from(h.as_str()).yellow());
            ...
            let table = Table::new(rows, headers.iter().map(|_| Constraint::Min(10)))
                ...
                .widths(&headers.iter().map(|_| Constraint::Min(10)).collect::<Vec<_>>());
